...
function Callout({ type = "info", children }) {
  const styles = {
    info: {
      ...
      icon: <Radio size={14} className="text-blue-500 shrink-0 mt-0.5" />,
      text: "text-blue-800",
    },
    warn: {
...
function Section({
  ...
  return (
    <div id={id} className="border rounded-xl overflow-hidden bg-white">
      ...
      {open && (
        <div className="px-5 pb-5 space-y-3 border-t text-sm text-gray-700 leading-relaxed">
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <a
      ...
      className={`block text-xs py-1 hover:text-indigo-600 transition ${
        sub
          ? "pl-4 text-gray-400 hover:text-indigo-500"
      ...
      <div className="flex-1 space-y-4 pb-20">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 max-w-xl">
            Everything you need to integrate, configure, and operate SmartHeal —
            the AI-powered self-healing agent for your applications.
          </p>
        </div>
        ...
        >
          <p>
            <strong>SmartHeal</strong> is an autonomous error-remediation
            platform. It watches your application for runtime errors, uses an
            LLM-powered agent to diagnose root causes, generates a patch,
            validates it through guardrails, and — if confidence is high enough
            ...
            {[
              {
                icon: Radio,
                title: "Detect",
                desc: "SDK intercepts console.error and ships errors to the backend in real-time.",
              },
              {
                icon: Cpu,
                title: "Diagnose",
                desc: "Agent runs root-cause analysis and generates a fix with a confidence score.",
              },
              {
                icon: GitBranch,
                title: "Heal",
                desc: "High-confidence fixes are committed to a new branch and a PR is opened automatically.",
              },
        ...
        >
          ...
          <p className="text-xs text-gray-500">
            Go to <strong>Projects</strong> in the sidebar → click{" "}
            <strong>New Project</strong> → fill in name and environments.
          </p>
          <p className="font-medium text-gray-800 mt-3">
            Step 2 — Copy your projectId
          </p>
          <CodeBlock language="text" code="proj_a1b2c3d4e5f6g7h8" />
          <p className="font-medium text-gray-800 mt-3">
            Step 3 — Install the SDK snippet
          </p>
          <CodeBlock
            language="javascript"
            code={`// Place this at the top of your app entry point
...
SmartHeal.init({
  ...
  projectId: "proj_a1b2c3d4e5f6g7h8",
          ...
          <p className="text-xs text-gray-500">
            In the Projects page, click <strong>Connect Repo</strong> on your
            project card → authorize with GitHub → select the repository.
          </p>
          <Callout type="success">
            That's it! SmartHeal will now detect errors, diagnose them, and open
            PRs automatically.
          </Callout>
        </Section>
        ...
        <Section id="sdk" icon={Terminal} title="SDK Integration">
          <p>
            The SmartHeal SDK is a tiny browser/Node.js shim that patches{" "}
            <code className="text-indigo-600 bg-indigo-50 px-1 rounded">
              console.error
            </code>{" "}
            and forwards captured errors to your SmartHeal backend.
          </p>
  ...
  silent:    false,           // true = don't log SDK activity to console
          ...
          <p className="text-xs text-gray-500">
            You can also send errors programmatically:
          </p>
          <CodeBlock
            language="javascript"
            code={`try {
  ...
  "env":       "prod"
          ...
          />
          <Callout type="warn">
            The SDK only captures <code>console.error</code> calls and uncaught
            promise rejections. It does NOT capture <code>console.warn</code> or{" "}
        ...
        <Section id="projects" icon={Layers} title="Projects & projectId">
          <p>
            A <strong>Project</strong> is the top-level resource in SmartHeal.
            Every error report must carry a{" "}
            <code className="text-indigo-600 bg-indigo-50 px-1 rounded">
              projectId
            </code>{" "}
            so the agent knows which GitHub repo to push fixes to.
          </p>

          <p className="font-medium text-gray-800 mt-4">projectId format</p>
          <CodeBlock
            language="text"
            code="proj_<16 hex chars>   e.g.  proj_04f73ed1812aa16c"
          />
              ...
              <thead>
                <tr className="bg-gray-50">
                  {["Field", "Type", "Description"].map((h) => (
                    <th
                      key={h}
                      className="border px-3 py-2 text-left font-semibold text-gray-600"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {[
                  ...
                  [
                    "envs",
                    "string[]",
                    "Environments this project covers (e.g. prod, staging, dev)",
                  ],
                  ["createdAt", "Date", "Creation timestamp"],
                ].map(([f, t, d]) => (
                  <tr key={f} className="border-t">
                    <td className="border px-3 py-2 font-mono text-indigo-600">
                      {f}
                    </td>
          ...
          <Callout type="info">
            Every <code>POST /agent/jobs</code> request is validated against the
            Projects collection. Requests with unknown projectIds are rejected
            with <code>404</code>.
          </Callout>
        </Section>
        ...
        <Section id="github" icon={GitBranch} title="GitHub Integration">
          <p>
            SmartHeal uses a <strong>GitHub OAuth App</strong> to create
            branches and open pull requests on your behalf.
          </p>

          <p className="font-medium text-gray-800 mt-4">OAuth flow</p>
          <ol className="list-decimal list-inside space-y-1 text-xs text-gray-600">
            <li>
              Click <strong>Connect Repo</strong> on a project card
            </li>
            <li>
              You're redirected to GitHub to authorize the SmartHeal OAuth App
            </li>
            <li>
              GitHub redirects back with <code>?step=select</code>
            </li>
            <li>
              Select a repository from the list — it's saved in the database
            </li>
          </ol>

          <p className="font-medium text-gray-800 mt-4">
            What SmartHeal creates
          </p>
          <CodeBlock
            language="text"
            code={`Branch:  smartheal-fix-<timestamp>
...
PR:      "🤖 SmartHeal Auto Fix" → base branch`}
          ...
          <Callout type="warn">
            Tokens are stored in MongoDB. For production use, encrypt tokens at
            rest using a KMS or secrets manager.
          </Callout>
        </Section>
        ...
        <Section id="agent" icon={Cpu} title="How the Agent Works">
          <p>
            When a job is picked up by the daemon, the following pipeline runs:
          </p>
            ...
            {[
              {
                step: "1",
                label: "Lock",
                desc: "Job atomically moves from NEW → PROCESSING via findOneAndUpdate.",
              },
              {
                step: "2",
                label: "Run Agent",
                desc: "LLM analyzes the error message, generates a fix + root cause analysis + confidence score (0–1). Retried once if confidence < 0.6.",
              },
              {
                step: "3",
                label: "Decision gate",
                desc: "confidence < 0.5 → REJECTED. 0.5–0.85 → AWAITING_APPROVAL (manual review). ≥ 0.85 → AUTO (continues).",
              },
              {
                step: "4",
                label: "Guardrails",
                desc: "Patch validated for security patterns, scope limits, syntax errors, and quality rules.",
              },
              {
                step: "5",
                label: "Create PR",
                desc: "New branch created, patch committed as a file, PR opened against the default branch.",
              },
              {
                step: "6",
                label: "Done",
                desc: "Job status → DONE, prUrl stored, alert sent.",
              },
            ].map(({ step, label, desc }) => (
              ...
              <tbody>
                {[
                  ...
                  [
                    "0.50 – 0.84",
                    "MANUAL",
                    "Job paused, awaits human approval",
                  ],
                  ...
                  <tr key={r} className="border-t">
                    ...
                    <td className="border px-3 py-2">
                      <code className="bg-gray-100 px-1 rounded">{d}</code>
                    </td>
        ...
        <Section id="guardrails" icon={Shield} title="Guardrails">
          <p>
            Before any patch is applied, it passes through four validation
            rules:
          </p>
            ...
            {[
              {
                name: "Security",
                file: "security.js",
                desc: "Blocks patches containing eval(), exec(), child_process, or dangerous shell patterns.",
              },
              {
                name: "Scope",
                file: "scope.js",
                desc: "Ensures the patch targets only source files (src/, app/, lib/) and doesn't touch config or infra files.",
              },
              {
                name: "Syntax",
                file: "syntax.js",
                desc: "Checks for balanced brackets, braces, and parentheses. Rejects incomplete code blocks.",
              },
              {
                name: "Quality",
                file: "quality.js",
                desc: "Enforces minimum patch length and rejects trivially empty or placeholder fixes.",
              },
            ].map(({ name, file, desc }) => (
              <div
                key={name}
          ...
          <Callout type="info">
            Guardrails are currently commented out in production to unblock the
            MVP. Re-enable in <code>daemon/watcher.js</code> by uncommenting the{" "}
            <code>validatePatch</code> block.
          </Callout>
        </Section>
        ...
        <Section id="jobs" icon={Radio} title="Issue Explorer">
          <p>
            The <strong>Issue Explorer</strong> page shows all jobs processed by
            SmartHeal with their full execution timeline.
          </p>
          ...
          <p className="text-xs text-gray-500">
            Each job has a timeline of steps. When a PR is created, a green
            banner appears at the top of the detail panel with a direct link to
            the GitHub PR.
          </p>

          <Callout type="info">
            Jobs auto-refresh every time you navigate to the Issue Explorer. For
            real-time updates, socket.io integration is on the roadmap.
          </Callout>
        </Section>
            ...
            <div key={path} className="border rounded-lg overflow-hidden mb-3">
              ...
              {body && (
                <div className="px-4 pt-2">
                  ...
                  <pre className="bg-gray-950 text-green-300 text-xs p-3 rounded overflow-x-auto">
                    {body}
                  </pre>
                </div>
              )}
              <div className="px-4 pt-2 pb-3">
                ...
                <pre className="bg-gray-950 text-blue-300 text-xs p-3 rounded overflow-x-auto">
                  {response}
                </pre>
              </div>
            </div>
        ...
        <Section id="faq" icon={BookOpen} title="FAQ">
          {[
            ...
            >
              ...
              <p className="px-4 pb-4 pt-1 text-xs text-gray-500 leading-relaxed border-t">
                {a}
              </p>
            </details>
          ))}
